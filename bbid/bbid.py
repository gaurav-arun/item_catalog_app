...
IMAGE_MD5s = {}
URLOPENHEADER = {'User-Agent': 'Mozilla/5.0 (X11; Fedora; Linux x86_64; '
...
def fetch_images_from_keyword(pool_sema: threading.Semaphore, keyword: str,
        ...
        try:
            ...
            for index, link in enumerate(links):
                if limit is not None and current + index >= limit:
                    return
                t = threading.Thread(target=download, args=(pool_sema,
                                                            link,
                                                            output_dir))
                t.start()
                current += 1
...
def fetch_random_image_from_keyword(keyword,
                                    output_dir=OUTPUT_DIR,
                                    filters=''):
    pool_sema = threading.BoundedSemaphore(1)
    current = 0
    request_url = 'https://www.bing.com/images/async?q=' + \
                  urllib.parse.quote_plus(keyword) + '&first=' + str(current) \
                  + '&count=35&adlt=' + ADLT + '&qft=' + \
...
def backup_history(*args):
    download_history = open(os.path.join(OUTPUT_DIR,
                                         'download_history.pickle'), 'wb')
